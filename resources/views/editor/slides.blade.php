...
<body class="h-full antialiased"
        ...
        <div class="flex items-center gap-2">
            ...
            <div class="relative">
                ...
                <div x-show="shareOpen"
                    ...
                    {{-- Header --}}
                    <div class="flex items-center gap-3 px-4 py-4" style="border-bottom:1px solid #f1f5f9;">
                        <div class="w-8 h-8 rounded-xl flex items-center justify-center shrink-0 bg-slate-100 border border-slate-200">
                        ...
                        <div>
                            <p class="text-sm font-semibold text-slate-900">Share this presentation</p>
                            <p class="text-xs text-gray-400 mt-0.5">Anyone with the link can view it</p>
                        </div>
                    </div>

                    {{-- URL --}}
                    <div class="px-4 py-3" style="border-bottom:1px solid #f1f5f9;">
                        <p class="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2">Public link</p>
                        <div class="flex items-center gap-2 px-3 py-2.5 rounded-xl bg-gray-50 border border-gray-200">
                            <svg class="w-3 h-3 shrink-0 text-gray-400" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M13.19 8.688a4.5 4.5 0 011.242 7.244l-4.5 4.5a4.5 4.5 0 01-6.364-6.364l1.757-1.757m13.35-.622l1.757-1.757a4.5 4.5 0 00-6.364-6.364l-4.5 4.5a4.5 4.5 0 001.242 7.244"/>
                            </svg>
                            <span class="text-xs font-mono text-gray-500 flex-1 truncate">{{ url('/s/' . $project->slug) }}</span>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-2 px-4 py-3">
                        <button @click="copyLink()"
                                ...
                                :class="linkCopied ? 'bg-green-600' : 'bg-slate-900 hover:bg-slate-800'">
                            <svg x-show="!linkCopied" class="w-3.5 h-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                            ...
                            <svg x-show="linkCopied" class="w-3.5 h-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5"/>
                            </svg>
                            <span x-text="linkCopied ? 'Copied!' : 'Copy link'"></span>
                        </button>
                        <a href="{{ url('/s/' . $project->slug) }}" target="_blank" rel="noopener"
                           class="flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold text-gray-600 hover:text-slate-900 bg-gray-100 hover:bg-gray-200 border border-gray-200 transition-colors">
    ...
    <div x-show="showExitModal"
        ...
        <div x-transition:enter="transition ease-out duration-150"
             ...
             class="relative w-full max-w-sm bg-white/95 backdrop-blur-xl border border-gray-200/80 rounded-2xl shadow-2xl overflow-hidden p-6">

            <div class="flex items-center justify-center w-11 h-11 rounded-xl bg-red-50 border border-red-100 mb-4">
            ...
            <p class="text-sm font-semibold text-slate-900 mb-1">You have unsaved changes</p>
            <p class="text-xs text-gray-400 leading-relaxed mb-5">Your presentation hasn't been saved yet. If you leave now, all unsaved changes will be lost.</p>

            <div class="flex items-center gap-2">
                <button @click="exitDiscard()"
                        class="flex-1 px-3 py-2 rounded-lg text-xs font-semibold text-red-600 bg-red-50 border border-red-100 hover:bg-red-100 hover:border-red-200 transition-colors">
                    Leave anyway
                </button>
                <button @click="exitSave()" :disabled="isSaving"
                        class="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold text-white bg-slate-900 hover:bg-slate-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                    <svg x-show="isSaving" class="w-3 h-3 animate-spin" fill="none" viewBox="0 0 24 24">
                ...
                <button @click="showExitModal = false"
                        class="px-3 py-2 rounded-lg text-xs font-semibold text-gray-500 hover:text-slate-900 hover:bg-gray-100 transition-colors">
                    Stay
                </button>
            </div>
        </div>
...
function editor() {
    return {
        ...
        init() {
            ...
            this.$watch('slides', () => { this._isDirty = true; });
        ...
        async exitSave() {
            const ok = await this.saveProject();
